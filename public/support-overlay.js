...
(function() {
  ...
  const STATUTS = {
    ...
    resolu:     { label:'Résolu',      color:'#0E4B47', bg:'#f0f7e8' },
  ...
  style.textContent = `
    #support-fab {
      ...
      background: linear-gradient(135deg,#0E4B47,#0E4B47);
    ...
    /* Head */
    .so-head {
      background: #0E4B47; padding: 18px 20px; display: flex; align-items: center;
    ...
    .so-tab { flex: 1; padding: 12px 8px; text-align: center; font-size: 12px; font-weight: 600; color: #B8D0C8; cursor: pointer; border-bottom: 2px solid transparent; transition: all 0.15s; display: flex; align-items: center; justify-content: center; gap: 5px; }
    .so-tab:hover { color: #0E4B47; background: #f0f7e8; }
    .so-tab.active { color: #0E4B47; border-bottom-color: #0E4B47; background: #fff; }
    ...
    .so-input:focus { border-color: #0E4B47; background: #fff; }
    ...
    .so-chip.sel-basse { background: #f0f7e8; border-color: #5ab52a; color: #0E4B47; }
    .so-meta-toggle { font-size: 11px; color: #B8D0C8; cursor: pointer; text-decoration: underline; }
    ...
    .so-submit { background: #0E4B47; color: #7BBBB5; border: none; border-radius: 10px; padding: 13px; font-size: 13px; font-weight: 700; font-family: inherit; cursor: pointer; transition: all 0.15s; display: flex; align-items: center; justify-content: center; gap: 7px; }
    ...
    .so-filter-pill { padding: 4px 10px; border-radius: 14px; border: 1px solid #e0ecd8; background: #fff; font-size: 11px; font-weight: 600; color: #B8D0C8; cursor: pointer; transition: all 0.12s; }
    .so-filter-pill.active { background: #0E4B47; color: #7BBBB5; border-color: #0E4B47; }
    ...
    .so-ticket-item.active { background: #f0f7e8; border-left: 3px solid #0E4B47; padding-left: 15px; }
    .so-ticket-top { display: flex; align-items: flex-start; justify-content: space-between; gap: 8px; }
    .so-ticket-id { font-size: 9px; font-weight: 700; color: #B8D0C8; letter-spacing: 0.5px; }
    ...
    .so-ticket-progress-fill { height: 100%; border-radius: 3px; background: linear-gradient(90deg,#5ab52a,#0E4B47); transition: width 0.4s; }
    ...
    .so-back { background: none; border: none; font-size: 12px; font-weight: 600; color: #B8D0C8; cursor: pointer; font-family: inherit; display: flex; align-items: center; gap: 4px; margin-bottom: 10px; padding: 0; transition: color 0.12s; }
    .so-back:hover { color: #0E4B47; }
    ...
    .so-progress-fill { height: 100%; border-radius: 6px; background: linear-gradient(90deg,#5ab52a,#0E4B47); transition: width 0.5s; }
    ...
    .so-sla.ok { background: #f0f7e8; color: #0E4B47; }
    ...
    .so-section-title { font-size: 10px; font-weight: 700; color: #B8D0C8; text-transform: uppercase; letter-spacing: 0.8px; margin-bottom: 8px; display: flex; align-items: center; justify-content: space-between; }
    ...
    .so-comment-avatar.admin { background: #0E4B47; }
    .so-comment-avatar.restaurant { background: #0E4B47; }
    ...
    .so-reply-input:focus { border-color: #0E4B47; background: #fff; }
    .so-reply-btn { width: 38px; height: 38px; border-radius: 9px; background: #0E4B47; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background 0.15s; flex-shrink: 0; }
    ...
    .so-meta-key { color: #B8D0C8; font-weight: 600; min-width: 80px; }
    ...
    .so-toast {
      ...
      background: #0E4B47; color: #7BBBB5; padding: 12px 18px;
  ...
  fab.innerHTML = `
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M12 17h.01M12 13a2 2 0 000-4 2 2 0 00-2 2" stroke="#7BBBB5" stroke-width="1.8" stroke-linecap="round"/>
      <circle cx="12" cy="12" r="9" stroke="#7BBBB5" stroke-width="1.5"/>
    </svg>
        ...
        <div class="so-view" id="so-view-new">
          <div class="so-form">
            ...
            <button class="so-submit" id="so-submit-btn" onclick="SupportOverlay.submit()">
              <svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path d="M2 10L18 2 10 18 9 11 2 10z" fill="#7BBBB5"/></svg>
              Envoyer la demande
            </button>
          </div>
        </div>
  ...
  function renderTicketList() {
    ...
    list.innerHTML = filtered.map(t => {
      ...
      const prioColors = { critique:'#fef0f0|#e05555', haute:'#fff8ec|#e08a10', normale:'#fffcec|#807010', basse:'#f0f7e8|#0E4B47' };
  ...
  function renderDetail(ticketId) {
    ...
    document.getElementById('so-detail-content').innerHTML = `
      ...
      <div class="so-detail-body">
        ...
        ${t.adminNote ? `<div><div class="so-section-title">📝 Note de l'équipe</div><div class="so-desc-block" style="background:#f0f7e8;border-color:#c8e8a0;color:#0E4B47">${t.adminNote.replace(/\n/g,'<br>')}</div></div>` : ''}
        <div>
  ...
  window.SupportOverlay = {
    ...
    async submit() {
      ...
      btn.disabled = false; btn.innerHTML = '<svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path d="M2 10L18 2 10 18 9 11 2 10z" fill="#7BBBB5"/></svg> Envoyer la demande';
      if (data.success) {
